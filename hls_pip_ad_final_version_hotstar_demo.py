...
class LocalAdCompositor:
    def __init__(self, main_url, stream_key, ad_urls=None, vast_url=None, ad_positions=None, ad_width=320, ad_height=180, interval=20):
    ...
    def _create_main_pipeline(self):
        ...
        pipeline_str = f"""
            ...
            x264enc bitrate=2500 speed-preset=ultrafast tune=zerolatency key-int-max=60 bframes=0 ! 
            h264parse ! flvmux name=mux streamable=true ! 
            rtmp2sink location="{rtmp_url}" async-connect=true
            
            uridecodebin uri="{self.main_url}" name=main_src
            
            main_src. ! queue name=main_video_queue ! videoconvert ! videoscale ! comp.sink_0
            
            main_src. ! queue name=main_audio_queue ! audioconvert ! audioresample ! 
            audio/x-raw,rate=44100,channels=2 !
            faac bitrate=128000 ! aacparse ! mux.
...
if __name__ == "__main__":
    ...
    HLS_AD_URLS = [
        "https://hesads.akamaized.net/ns/1767771552970/03d26766f2297d58aa12b4f82e1f3d17/master.m3u8",
        "https://hesads.akamaized.net/ns/1767093314760/a8ee1b5a80baa5f3db38f8187af499f6/master.m3u8",
        "https://hesads.akamaized.net/ns/1764835239310/c0cafeaf834c3063414fc2371c58625b/master.m3u8",
        "https://hesads.akamaized.net/ns/1764835239310/c0cafeaf834c3063414fc2371c58625b/master.m3u8"
    ]
    
    # India Today Live HLS URL (Sister channel of Aaj Tak, confirmed working in original script)
    MAIN_HLS_URL = "https://indiatodaylive.akamaized.net/hls/live/2014320/indiatoday/indiatodaylive/live_720p/chunks.m3u8"
    
    STREAM_KEY = "qq2f-wraq-5ttx-yurb-cxbf"
    
    positions = [(920, 500), (40, 500), (40, 40), (920, 40)]
    
    player = LocalAdCompositor(main_url=MAIN_HLS_URL, stream_key=STREAM_KEY, ad_urls=HLS_AD_URLS, ad_positions=positions)
    player.run()
