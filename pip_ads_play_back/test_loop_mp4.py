...
class VideoCompositor:
    ...
    def __init__(self, main_url, overlay_url, overlay_x=640, overlay_y=360, overlay_width=320, overlay_height=180):
        ...
        self.pipeline = None
        self.looped_main_file = None
        self.looped_overlay_file = None

    def _create_looped_video(self, input_url, loop_count=100):
        """Pre-create a looped version of a video using FFmpeg"""
        print(f"[SETUP] Creating looped video ({loop_count} loops): {input_url[:50]}...")

        # Create temp file
        looped_file = tempfile.mktemp(suffix=".mp4")

        # Use FFmpeg to loop the video with proper audio handling
        cmd = [
            "ffmpeg", "-y",
            "-stream_loop", str(loop_count - 1),
            "-i", input_url,
            "-c:v", "copy",
            "-c:a", "aac",  # Re-encode audio to ensure proper looping
            "-b:a", "128k",
            "-t", "3600",  # Max 1 hour
            "-shortest",
            looped_file
        ...
        try:
            ...
            if result.returncode == 0:
                print(f"[SETUP] Looped video created: {looped_file}")
                return looped_file
            else:
    ...
    def _create_pipeline(self):
        ...
        # Use looped files if available, otherwise use original
        main_uri = f"file://{self.looped_main_file}" if self.looped_main_file else self.main_url
        ...
        pipeline_str = f"""
            ...
            uridecodebin uri="{main_uri}" name=main_src !
    ...
    def run(self):
        ...
        print("[COMPOSITOR] Video Compositor with Looping Videos")
        ...
        # Create looped versions of both videos
        self.looped_main_file = self._create_looped_video(self.main_url, loop_count=50)
        if not self.looped_main_file:
            print("[WARNING] Using original main video (may not loop)")

        self.looped_overlay_file = self._create_looped_video(self.overlay_url, loop_count=100)
        if not self.looped_overlay_file:
        ...
        try:
            ...
            # Cleanup temp files
            for f in [self.looped_main_file, self.looped_overlay_file]:
